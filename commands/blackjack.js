...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      // Clear stale ended session if somehow still cached
      ...
      // --- NEW SESSION CREATION (FIXED) ---
      // If host provided bet, validate affordability BEFORE creating any panel/collector.
      if (bet != null) {
        if (bet < MIN_BET) {
          return interaction.editReply(`❌ Minimum bet is $${MIN_BET.toLocaleString()}.`);
        }

        // Precheck: try to debit first. If it fails, don't create the lobby panel at all.
        const debit = await tryDebitUser(guildId, interaction.user.id, bet, "blackjack_buyin", {
          channelId,
          preStart: true,
        });
        ...
        // Create lobby now that we know the host bet is paid
        ...
        // Wire collector (so buttons never fail)
      ...
      // No bet provided: normal lobby creation
      const session = new BlackjackSession({
        channel: interaction.channel,
        hostId: interaction.user.id,
        guildId,
...
function wireCollectorHandlers({ collector, session, interaction, guildId, channelId }) {
  async function handleGameEnd() {
    ...
    // Pay pushes first, then wins, then losers
    const pushes = payoutPlan.filter((p) => p.result === "push" && p.payoutWanted > 0);
    const wins = payoutPlan.filter(
      (p) => (p.result === "win" || p.result === "blackjack_win") && p.payoutWanted > 0
    );
    ...
    for (const p of ordered) {
      ...
      if (p.payoutWanted > 0) {
        ...
        if (full.ok) {
          ...
          // If payout > base bet, fallback to refund base bet
  ...
  collector.on("collect", async (i) => {
    ...
    if (action === "join") {
      ...
      if (!res.ok) {
        return i
          .followUp({ content: `❌ ${res.msg}`, flags: MessageFlags.Ephemeral })
          .catch(() => {});
      }
      await session.updatePanel();
      return i
    ...
    if (action === "leave") {
      ...
      // If they already paid, try refund from bank
      if (wasPaid) {
